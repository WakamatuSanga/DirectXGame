...
//
	...
	//swapChainDesc.Width =
	//	WinApp::kClientWidth; // 画面の幅。ウィンドウのクライアント領域を同じものんにしておく
	//swapChainDesc.Height =
	//	WinApp::kClientHeight; // 画面の高さ。ウィンドウのクライアント領域を同じものにしておく
	...
	assert(SUCCEEDED(hr));
	// ===== Sprite/Particle instancing 用 RootSignature =====
	...
	instRange.BaseShaderRegister = 0; // t0
	...
	sprRootDesc.pStaticSamplers = staticSamplers;            // 既存のサンプラを流用
	sprRootDesc.NumStaticSamplers = _countof(staticSamplers);

	ID3DBlob* sprSigBlob = nullptr;
	ID3DBlob* sprErr = nullptr;
	hr = D3D12SerializeRootSignature(&sprRootDesc, D3D_ROOT_SIGNATURE_VERSION_1, &sprSigBlob, &sprErr);
	if (FAILED(hr)) {
		Log(logStream, reinterpret_cast<char*>(sprErr->GetBufferPointer()));
		assert(false);
	}
	ID3D12RootSignature* spriteRootSignature = nullptr;
	hr = device->CreateRootSignature(0, sprSigBlob->GetBufferPointer(), sprSigBlob->GetBufferSize(),
...
//#pragma endregion
	...
	vertexDataSprite[3].position = { 0.5f,  0.5f, 0.0f, 1.0f }; // 右上
	...
	instancingSrvGPU = GetGPUDescriptorHandle(srvDescriptorHeap, descriptorSizeSRV, 3);
...
// 1. 通常ブレンド(Normal)
	...
	// ===== PSO for Sprite/Particle instancing =====
	D3D12_GRAPHICS_PIPELINE_STATE_DESC sprPsoDesc = graphicsPipelineStateDesc; // 既存をベース
	...
	hr = device->CreateGraphicsPipelineState(&sprPsoDesc, IID_PPV_ARGS(&psoSpriteParticles));
	assert(SUCCEEDED(hr));
	...
	while (true) {
		...
		if (winApp->ProcessMessage()) {
			...
			if (!uiWantsMouse && input.MouseDown(Input::MouseLeft)) {
				const float sensitivity = 0.0025f;      // マウス感度（ラジアン/ピクセル）
				cameraTransform.rotate.y -= input.MouseDeltaX() * sensitivity; // yaw（左右）
				cameraTransform.rotate.x += input.MouseDeltaY() * sensitivity; // pitch（上下）

				// ピッチを ±89° に制限（見上げ過ぎ/見下ろし過ぎ防止）
				const float kPitchLimit = 1.55334306f;  // 89deg in rad
				cameraTransform.rotate.x = std::clamp(cameraTransform.rotate.x, -kPitchLimit, kPitchLimit);
			}
			...
			float speed = input.PushKey(DIK_LSHIFT) ? 6.0f : 2.5f; // Shiftでダッシュ
			const float dt = 1.0f / 60.0f;

			// Yaw（Y回転）を基準に、XZ平面で移動ベクトルを回す（左手系：+Zが前）
			if (move.x != 0.0f || move.z != 0.0f) {
				const float yaw = cameraTransform.rotate.y;
				Vector3 forward = { std::sin(yaw), 0.0f, std::cos(yaw) };   // 前
				Vector3 right = { std::cos(yaw), 0.0f,-std::sin(yaw) };   // 右
				Vector3 delta = {
			...
			// RootParams: [0]=Material(b0), [1]=Instances(t0 VS), [2]=Texture(t0 PS)
			commandList->SetGraphicsRootConstantBufferView(0, materialResourceSprite->GetGPUVirtualAddress());
			commandList->SetGraphicsRootDescriptorTable(1, instancingSrvGPU);

			// 好きなテクスチャに。ここではチェック柄（textureSrvHandleGPU）を使用
			commandList->SetGraphicsRootDescriptorTable(2, textureSrvHandleGPU);
			...
			//// SRVヒープの3番を利用（0:ImGui, 1/2:既存のテクスチャを想定）
			//D3D12_SHADER_RESOURCE_VIEW_DESC instancingSrvDesc{};
			//instancingSrvDesc.Format = DXGI_FORMAT_UNKNOWN;
			//instancingSrvDesc.Shader4ComponentMapping = D3D12_DEFAULT_SHADER_4_COMPONENT_MAPPING;
			//instancingSrvDesc.ViewDimension = D3D12_SRV_DIMENSION_BUFFER;
...
// --- 同期・イベント系 ---
	...
	// WindowsAPI解放
	dxCommon->Finalize();   // 呼び出しだけ追加（中身が空でもOK）
	delete dxCommon;
	delete winApp;
	winApp = nullptr;
	CoUninitialize();
