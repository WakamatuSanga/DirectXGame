...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
	...
	HWND hwnd = CreateWindow(
		...
		L"CG2_LE2D_26_ワカマツ_サンガ",	//タイトルバーの文字
	...
	*materialData = Vector4(1.0f, 0.0f, 0.0f, 1.0f);
	...
	// RootParameter作成。複数設定できるので配列。今回は結果が１つなので長さ１の配列
	...
	// InputLayout
	D3D12_INPUT_ELEMENT_DESC inputElemenDescs[1] = {};
	inputElemenDescs[0].SemanticName = "POSITION";
	inputElemenDescs[0].SemanticIndex = 0;
	inputElemenDescs[0].Format = DXGI_FORMAT_R32G32B32A32_FLOAT;
	inputElemenDescs[0].AlignedByteOffset = D3D12_APPEND_ALIGNED_ELEMENT;
	D3D12_INPUT_LAYOUT_DESC inputLayoutDesc{};
	inputLayoutDesc.pInputElementDescs = inputElemenDescs;
	inputLayoutDesc.NumElements = _countof(inputElemenDescs);
	// BlendStateの設定
